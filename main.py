...
def clean_cache():
    if 'cache' in os.listdir():
        location = os.getcwd()
        path = os.path.join(location, 'cache')
...
zip_file = os.path.join(location, 'files') + "/data.zip"
print(location)
...
# define and print file paths
list_cached_files = cached_files()
print(list_cached_files)


# Find the password
def find_password(cached_files=list_cached_files):
...
# Print password
print(find_password(list_cached_files))
